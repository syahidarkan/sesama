...
export default function HomePage() {
  ...
  // Format compact number for banner (e.g. 1500000 → "1,5Jt", 50000000 → "50Jt")
  const formatCompact = (amount: number) => {
    ...
    if (amount >= 1_000) return `${(amount / 1_000).toFixed(1).replace('.0', '')}Rb`;
    return amount.toString();
  };

  const formatDonors = (count: number) => {
    if (count >= 1_000_000) return `${(count / 1_000_000).toFixed(1).replace('.0', '')}Jt+`;
    if (count >= 1_000) return `${Math.floor(count / 1_000)}Rb+`;
    return `${count}`;
  };
