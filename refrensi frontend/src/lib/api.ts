...
export const referralsApi = {
  // Generate a referral code (optionally for a specific program)
  generateCode: (programId?: string) =>
    api.post('/referrals/generate', null, { params: { programId } }),

  // Get referral code details by code string
  getByCode: (code: string) => api.get(`/referrals/code/${code}`),

  // Get all my referral codes
  getMyCodes: () => api.get('/referrals/my-codes'),
  ...
  getLeaderboard: (limit = 10, offset = 0) =>
    api.get('/referrals/leaderboard', { params: { limit, offset } }),

  // Deactivate a referral code
  deactivateCode: (codeId: string) => api.delete(`/referrals/code/${codeId}`),

  // Build referral link URL
  buildReferralLink: (code: string, programSlug?: string) => {
    const baseUrl = typeof window !== 'undefined' ? window.location.origin : '';
    if (programSlug) {
      return `${baseUrl}/programs/${programSlug}?ref=${code}`;
    }
